...
for experiment_no=1:n
    ...
    % Cost graph
    z = length(cost_train(experiment_no,:));
    plot(update_steps_numbering(1:z),cost_train(experiment_no,:));
    hold on;
    plot(update_steps_numbering(1:z),cost_validate(experiment_no,:));
    ...
    % Loss graph
    h = subplot(2,2,2);
    plot(update_steps_numbering(1:z),loss_train(experiment_no,:));
    hold on;
    plot(update_steps_numbering(1:z),loss_validate(experiment_no,:));
    legend('Training set','Validation set');
    xlabel('Training steps');
    ylabel('Loss');
    ...
    % Accuracy graph
    h = subplot(2,2,3);
    plot(update_steps_numbering(1:z),accuracy_train(experiment_no,:));
    hold on;
    plot(update_steps_numbering(1:z),accuracy_validate(experiment_no,:));
    legend('Training set','Validation set');
    xlabel('Training steps');
    ylabel('Accuracy (%)');
