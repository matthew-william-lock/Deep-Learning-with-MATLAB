function Grads = ComputeGradsNumSlowBatchNormAfterNum(X, Y, NetParams, lambda, h)
...
for j=1:length(NetParams.b)
    ...
    for i=1:length(NetParams.b{j})
        ...
        NetTry.b = b_try;
        c1 = ComputeCostBatchNormAfterRelu(X, Y, NetTry, lambda);        
        
        b_try = NetParams.b;
        b_try{j}(i) = b_try{j}(i) + h;
        NetTry.b = b_try;        
        c2 = ComputeCostBatchNormAfterRelu(X, Y, NetTry, lambda);
