function [W,B,gammas,betas] = InitParametersBatchNorm(X_test,Y_train,d,intermediate_layers)
    ...
    for i = 1: length(intermediate_layers)    
        ...
        if i==1
            W(i) = {normrnd(0,1/sqrt(d),[intermediate_layers(i),size(X_test,1)])};
            B(i) = {zeros([intermediate_layers(i),1])};
        else
            W(i) = {normrnd(0,1/sqrt(intermediate_layers(i-1)),[intermediate_layers(i),intermediate_layers(i-1)])};
            B(i) = {zeros([intermediate_layers(i),1])};
        end    
